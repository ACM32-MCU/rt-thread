...
{
//	if (hlpuart->Instance->SR & BIT8)
//	{
//		hlpuart->Instance->SR |= BIT8;
//	}
...
//            fbaud = 0x952; 
        ...
        case 1500000:   //PCLK = 48M  lpuart_clk = PCLK/32=1.5MHz
		switch(baud_rate) 
		{
			case 115200:    //1.5M/115200=13.021  
			ibaud = 12;     //ibaud=(13-1)
			fbaud = 0x000;  // 0.021 *11=0
			break;
			
			case 9600:      //1.5M/9600=156.25
			ibaud = 155;    //ibaud=(156-1)
			fbaud = 0x888;  // 0.25 *11= 3
			break;
		}
        rxsamp = ibaud >> 1;    
        break;
...
void LPUART_Clock_Select(uint8_t lpuart_clk_src)
{
    if (0 == lpuart_clk_src)
    {
        RCC->CCR2 &= (~(BIT16 | BIT17) );     // RC32K 
    }
    else if (1 == lpuart_clk_src) 
    {
        RCC->CCR2 = (RCC->CCR2 & (~(BIT16 | BIT17) )) | (BIT16);     // XTAL 
    }
    else
    {
        RCC->CCR2 = (RCC->CCR2 & (~(BIT16 | BIT17) )) | (BIT17);     // PCLK
        RCC->CCR2 = (RCC->CCR2 & (~(BIT14 | BIT15) )) | (BIT14 | BIT15);  // pclk/32          
    }
}
/************************************************************************
