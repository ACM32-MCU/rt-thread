...
HAL_StatusTypeDef HAL_CAN_Init(CAN_HandleTypeDef *hcan)
{
    ...
    hcan->Instance->BTR=(hcan->Init.CAN_SJW<<14)|(hcan->Init.CAN_BRP<<8)|(hcan->Init.CAN_SAM<<7)|(hcan->Init.CAN_TSEG2<<4)|(hcan->Init.CAN_TSEG1);
    HAL_CAN_OperatingModeRequest(hcan,hcan->Init.CAN_Mode);
    return HAL_OK;
}
...
HAL_StatusTypeDef HAL_CAN_Transmit(CAN_HandleTypeDef *hcan, CanTxRxMsg* TxMessage)
{
    ...
    if(hcan->Init.CAN_Mode==CAN_OperatingMode_LOOPBACK||hcan->Init.CAN_Mode==CAN_OperatingMode_SILENT||hcan->Init.CAN_Mode==CAN_OperatingMode_SILENT_LOOPBACK)
    {
        hcan->Instance->CMR = CAN_CMR_SRR;  // transfer request CAN_CMR_SRR  CAN_CMR_TR
    }
    else
    {
        hcan->Instance->CMR = CAN_CMR_TR;  // transfer request CAN_CMR_SRR  CAN_CMR_TR
    }
    ...
    if (CAN_FilterInitStruct->FilterMode ==CAN_FilterMode_Dual) /*Dual mode*/
    {
        ...
        /*Dual mode AMR set*/
         hcan->Instance->FILTER[filter_num+7] =
                                ((CAN_FilterInitStruct->FilterMaskId1&0xFFFF0000)>>16)|         
                                (CAN_FilterInitStruct->FilterMaskId2&0xFFFF0000) ;          	       
    }
    else /*Single mode*/
    {
        hcan->Instance->MOD |= (1<<(8+filter_num));
        ...
        /*Single mode AMR set*/   
         hcan->Instance->FILTER[filter_num+7] = CAN_FilterInitStruct->FilterMaskId1;           
    }
